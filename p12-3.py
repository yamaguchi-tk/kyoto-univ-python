...
#
      ...
      self.show_sec = False
      self.timetext = time.strftime('%Y/%m/%d %I:%M:%S(%p)')
  ...
  def display_time(self):
    ...
    self.clock.create_text(x, y, text=self.timetext, font=("", 12), fill="black", tag="TIME")
    if self.show_date:
      self.clock.create_text(x, y, text=self.timetext, font=("", 12), fill="black", tag="TIME")
  ...
  def display(self):
    ...
    # 秒針の描画
    #
    if self.sec != time.localtime().tm_sec :
      self.display_sec()
    ...
    if self.min != time.localtime().tm_min :
      self.display_min()

    if self.hour != time.localtime().tm_hour :
      self.display_hour()
    #
    # 日付の描画、秒が変わるか、ボタンが押された時
    #
    if self.timetext != time.strftime('%Y/%m/%d %I:%M:%S(%p)') :
      self.display_time()
    #
    # 100ミリ秒後に再度呼び出す
    #
